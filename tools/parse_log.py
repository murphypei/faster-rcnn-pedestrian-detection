...
def parse_log(path_to_log):
    ...
    with open(path_to_log) as f:
        ...
        for line in f:
            ...
            # print "train_dict_list", train_dict_list
...
def parse_line_for_net_output(regex_obj, row, row_dict_list, line, iteration,
    ...
    if output_match:
        if not row or row['NumIters'] != iteration:
            ...
            row = OrderedDict([('NumIters', iteration), ('Seconds', seconds),
...
def save_csv_files(logfile_path,
                   output_dir,
                   train_dict_list,
                   test_dict_list,
                   delimiter=',',
    ...
    print "save train filename: ", train_filename
...
def write_csv(output_filename, dict_list, delimiter, verbose=False):
    ...
    with open(output_filename, 'w') as f:
        dict_writer = csv.DictWriter(
            f, fieldnames=dict_list[0].keys(), dialect=dialect)
...
def parse(logfile_path, output_dir, delimiter=','):
    train_dict_list, test_dict_list = parse_log(logfile_path)

    return save_csv_files(
        logfile_path,
        output_dir,
        train_dict_list,
        test_dict_list,
